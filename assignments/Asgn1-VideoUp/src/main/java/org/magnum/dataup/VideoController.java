...
public class VideoController {
    ...
    public @ResponseBody Collection<Video> getVideos() {
        Collection<Video> videosList = new ArrayList<Video>();
        for (long id : videos.keySet()) {
            videosList.add(videos.get(id));
        }
    ...
    private Video save(Video entity) {
        ...
        // Update video object with generated data URL
        entity.setDataUrl(getDataUrl(entity.getId()));

        // Store updated video object in memory
    ...
    private String getDataUrl(long videoId) {
        ...
        String urlBaseForLocalServer = "http://"+request.getServerName()
                + ((request.getServerPort() != 80) ? ":"+request.getServerPort() : "");

        return urlBaseForLocalServer + "/video/" + videoId + "/data";
    }
}
